                ...
                return (
                    <Col key={filteredData.id}>
                        <Card bg="light" border="light">
                            <img src={filteredData.images.small} alt="Pokemon card." />
                            <Card.Subtitle className="m-2">{filteredData.name}</Card.Subtitle>
                            <Button>
                                <Link to={`cards/${filteredData.id}`}>View</Link>
                            </Button>
                        </Card>
                    </Col>
                )
...
/**    const isGrassType = cards.types;

    if (isGrassType = "Grass") {
      return <CardList />;
    }
        return [];
    } */
